...
class ZopClient
{
    ...
    public function execute($zopRequest)
    {
        if ($zopRequest->getBody() == null) {
            ...
            $str_to_digest = substr($str_to_digest, 0, -1)
                .$this->zopProperties->getKey();
            ...
            return $this->httpClient->post(
                $url, $headers, http_build_query($fixedParams), 0
            );
        } else {
            $url           = $zopRequest->getUrl();
            ...
            return $this->httpClient->post($url, $headers, $body, 0);
