...
class Message
{
    public $url = 'http://120.55.197.77:1210';
    public $userCode = 'QDRHCF';
    public $userPass = 'Yunxin123';
    public $chanel = 0; //默认为0.国际短信为999
    ...
    public function sendMessage($mobile, $message,
        $method = "/Services/MsgSend.asmx/SendMsg"
    ) {
        $param                      = ["userCode" => $this->userCode,
                                       "userPass" => $this->userPass,
                                       "DesNo"    => $mobile, 'Msg' => $message,
                                       'Channel'  => $this->chanel];
        $url                        = $this->url.$method;
        $ch                         = curl_init();
        ...
        if (isset($this->_message[$dataArr[0]])) {
            return $this->_message[$dataArr[0]]['msg'] ??
                '原因不明：'.json_encode($dataArr, JSON_UNESCAPED_UNICODE);
        } else {
            return true;
        }
    }
